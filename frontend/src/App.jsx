...
function App() {
  ...
  const moneyAction = (type, account) => setModal({ type: 'money', action: type, account })
  ...
  return <div className="app-shell">
    ...
    <main className="main-content"><header className="topbar"><div><p className="eyebrow">{activeView === 'Overview' ? 'Thursday, September 17, 2026' : 'Bankify workspace'}</p><h1>{activeView === 'Overview' ? 'Good morning' : activeView}</h1></div><div className="user-area"><div className="user-avatar">{credentials.username?.slice(0, 1).toUpperCase()}</div><div><strong>{credentials.username}</strong><span>Account manager</span></div><button className="more-button" onClick={logout}>•••</button></div></header>
